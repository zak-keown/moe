...
export function runShow(args) {
    ...
    for (let i = 0; i < args.length; i++) {
        ...
        if (arg === "--format" || arg === "-f") {
            ...
            if (value !== "markdown" && value !== "html") {
                console.error(`Invalid --format value: ${value ?? "(missing)"}. Expected markdown or html.`);
                return 1;
            }
            format = value;
        }
    ...
    try {
        ...
        console.log(format === "html" ? formatConversationAsHTML(jsonl) : formatConversationAsMarkdown(jsonl));
        return 0;
    }
    catch (error) {
