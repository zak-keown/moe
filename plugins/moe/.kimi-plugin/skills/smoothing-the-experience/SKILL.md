...
1. Use `$SKILL` as the path prefix for script invocations below.
2. Run `node "$SKILL/smooth.mjs" scan --days 30`.
...
5. Run `node "$SKILL/smooth.mjs" plan --select <comma-separated-ids>` for exactly one harness and show the exact diff.
...
8. Pass the plan path and printed confirmation token to `node "$SKILL/smooth.mjs" apply --plan <path> --confirm <token>`. Report whether the change applied and whether the user must start a new session. Leave every other harness unchanged.
