...
triggers: >-
  ...
  exists and needs execution (`sdd`).
---
...
**At this depth:** `write-plan` fires only at the `feature` depth
...
**Context:** If working in an isolated worktree, it should have been created via the `use-worktrees` skill at execution time.
...
`sequence-plans` skill covers the manifest format and the `plan-set` CLI.
...
`retrieve-context` skill or the `moe:search-moedex` agent), query
...
[skills/sdd/scripts/task-set.mjs](../sdd/scripts/task-set.mjs) relative to
...
`sdd` (Wave grouping, Integrate the wave) and
`dispatch-agents` (Validate the plan before applying the gate, The
