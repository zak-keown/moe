...
Use this prompt with a **a fast model from the current spawn allowlist** subagent for cost-effective categorization.
...
1. Run extraction: resolve [skills/find-duplicates/scripts/extract-functions.mjs](extract-functions.mjs) relative to this loaded document, then run `node "<resolved-extract-functions.mjs>" src/ -o catalog.json`
2. Dispatch a fast model from the current spawn allowlist subagent with the prompt above, replacing:
...
**Critical:** The subagent must use `apply_patch` to save output. If it only returns a summary, re-prompt with explicit file write instructions.
