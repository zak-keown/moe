...
| 2. Categorize | `scripts/categorize-prompt.md` | a fast model from the current spawn allowlist | `categorized.json` |
| 3. Split | `scripts/prepare-category-analysis.mjs` | - | `categories/*.json` |
| 4. Detect | `scripts/find-duplicates-prompt.md` | a deep-reasoning model from the current spawn allowlist | `duplicates/*.json` |
...
Resolve [skills/finding-duplicate-functions/scripts/extract-functions.mjs](scripts/extract-functions.mjs) relative to this loaded document, then invoke it as:
...
Dispatch a **a fast model from the current spawn allowlist** subagent using [skills/finding-duplicate-functions/scripts/categorize-prompt.md](scripts/categorize-prompt.md), resolved relative to this loaded document.
...
Resolve [skills/finding-duplicate-functions/scripts/prepare-category-analysis.mjs](scripts/prepare-category-analysis.mjs) relative to this loaded document, then invoke it as:
...
For each category file in `./categories/`, dispatch a **a deep-reasoning model from the current spawn allowlist** subagent using [skills/finding-duplicate-functions/scripts/find-duplicates-prompt.md](scripts/find-duplicates-prompt.md), resolved relative to this loaded document.

Save each output as `./duplicates/<category-name>.json`.

### Phase 5: Generate Report

Resolve [skills/finding-duplicate-functions/scripts/generate-report.mjs](scripts/generate-report.mjs) relative to this loaded document, then invoke it as:
...
rung 2 (the brainstorm browser companion)
and fall through to rung 3 (local HTML file) or rung 4 (markdown file)
when the sandbox blocks a port bind. Use the shared ladder's sharing
default.
...
**Using a fast model from the current spawn allowlist for duplicate detection**: the fast role is cost-effective for categorization but misses subtle semantic duplicates. Use a deep-reasoning model from the current spawn allowlist for the actual duplicate analysis.
