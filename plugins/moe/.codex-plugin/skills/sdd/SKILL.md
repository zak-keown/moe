...
triggers: >-
  ...
  initial planning (`write-plan`), or low-level dispatch mechanics
  without a plan (`dispatch-agents`).
---

# Subagent-Driven Development

**At this depth:** `sdd` runs a plan produced
by `write-plan`, which fires only at the `feature` depth defined in
...
digraph process {
    ...
    "Dispatch final code reviewer (../request-review/code-reviewer.md)" [shape=box];
    ...
    "More tasks remain?" -> "Dispatch final code reviewer (../request-review/code-reviewer.md)" [label="no"];
    "Dispatch final code reviewer (../request-review/code-reviewer.md)" -> "Final findings? ONE fix dispatch, one scoped re-review, adjudicate residuals";
...
`use-worktrees` to create one or verify the existing one.
...
- Each plan owns a workspace: at skill start, resolve
  [skills/sdd/scripts/sdd-workspace.mjs](scripts/sdd-workspace.mjs) relative
  to this loaded document and invoke it as `node "<resolved-sdd-workspace.mjs>" PLAN_FILE`
...
- Check for this plan's ledger at `<workspace>/progress.md`. If its first
  ...
  ledger at the old flat path `.moe/sdd/progress.md` — is another
...
(`dispatch-agents`, "The gate"): pairwise-disjoint `Files:` blocks,
...
waves deterministically. Resolve [skills/sdd/scripts/task-set.mjs](scripts/task-set.mjs) relative to this loaded document, then invoke it as:
...
`dispatch-agents`. A worker branched from a stale base will cite the
...
**Mechanical implementation tasks** (isolated functions, clear specs, 1-2 files): use a fast model from the current spawn allowlist. Most implementation tasks are mechanical when the plan is well-specified.

**Integration and judgment tasks** (multi-file coordination, pattern matching, debugging): use the configured default.

**Architecture and design tasks**: use a deep-reasoning model from the current spawn allowlist.
The final whole-branch review is one of these — dispatch it with
a deep-reasoning model from the current spawn allowlist, not an unexamined session default.

**Review tasks**: use the configured default for ordinary reviews and a deep-reasoning model from the current spawn allowlist
when the diff's size, complexity, or risk needs deeper judgment. Scoped
re-reviews of small mechanical fix diffs may use a fast model from the current spawn allowlist.
...
implementation is transcription plus testing: use a fast model from the current spawn allowlist for
that implementer. Single-file mechanical fixes also use a fast model from the current spawn allowlist.

**Task complexity signals (implementation tasks):**
- Touches 1-2 files with a complete spec → a fast model from the current spawn allowlist
- Touches multiple files with integration concerns → the configured default
- Requires design judgment or broad codebase understanding → a deep-reasoning model from the current spawn allowlist
...
- **Task brief:** before dispatching an implementer, resolve
  [skills/sdd/scripts/task-brief.mjs](scripts/task-brief.mjs) relative to
  this loaded document and invoke it as
  `node "<resolved-task-brief.mjs>" PLAN_FILE N` — it extracts the task's full text
...
- Dispatch multiple implementers concurrently ONLY on rung one: the wave's
  ...
  pairwise-unique linked Git directory via `use-worktrees` Step 1d. Rung
...
**DONE:** Resolve [skills/sdd/scripts/review-package.mjs](scripts/review-package.mjs) relative to this loaded document and invoke it as `node "<resolved-review-package.mjs>" PLAN_FILE BASE HEAD`. It prints the unique file path it wrote; BASE is the commit you recorded before dispatching the implementer — never `HEAD~1`, which silently drops all but the last commit of a multi-commit task. Then dispatch the task reviewer with the printed path.
...
- Hand the reviewer its diff as a file: invoke the resolved
  `review-package.mjs` resource with `PLAN_FILE BASE HEAD` and pass the reviewer the file path
...
**The re-review is scoped.** Invoke the resolved `review-package.mjs` resource with `PLAN_FILE FIX_BASE HEAD`
...
`review-package.mjs` resource with `PLAN_FILE MERGE_BASE HEAD` (MERGE_BASE = the commit the
branch started from, e.g. `git merge-base main HEAD`) and include the
printed path in the final review dispatch, so the final reviewer reads
one file instead of re-deriving the branch diff with git commands. Dispatch
with a deep-reasoning model from the current spawn allowlist (see Model Selection), using
`request-review`'s
[code-reviewer.md](../request-review/code-reviewer.md). Point it at
...
(invoke the resolved `review-package.mjs` resource with `PLAN_FILE FIX_BASE HEAD` over the fix range,
...
[Resolve the linked sdd-workspace.mjs resource relative to this document and invoke it with docs/moe/plans/feature-plan.md — no ledger inside, fresh start]
...
You: "User level (~/.config/moe/hooks/)"
