...
Use `shell` (curl) on that URL. One page, current, no cache to go stale.
...
`shell` (rg/grep), populate a local cache first:

Resolve [skills/cc-config/scripts/update_docs.mjs](scripts/update_docs.mjs)
...
directory. Then use `shell` (cat) or `shell` (rg/grep) on that directory as usual.
