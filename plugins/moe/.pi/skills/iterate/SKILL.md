...
triggers: >-
  ...
  well-scoped features with clear requirements (`write-plan` then
  `sdd`), single tasks, bug fixes, or projects
...
- The write-plan flow has lost the plot on this project before

Do NOT use for a `patch` or a `change` (see `brainstorming`) — the `write-plan` → `sdd` flow is simpler and more appropriate for those depths.
...
2. Establish one project workspace with `use-worktrees`. Record its
   ...
   `implement-tasks` integrates back into this workspace.
3. Invoke `extract-requirements` on the human-provided spec path.
...
4. Invoke `scope-core` on the resulting backlog.
...
while True:
    ...
    run next iteration:
        - run-iteration (sentinel baseline → scope review → decompose code + evidence tasks → implement-tasks → impacted + sentinel scenario runs → wrap up)

    audit:
        - audit-progress (PAR paired auditors, three-tier: deep evidence + impacted behavior + sentinel corpus)
...
When the answer is yes, invoke `finish-branch` exactly once,
...
- At the boundary: invoke `extract-requirements` in incremental mode on the changed spec files, merge new/revised story cards into the backlog, revise the roadmap if changes invalidate downstream iterations, then resume
...
When running autonomously, this orchestrator takes precedence over interactive-gate skills (e.g., `brainstorming` which requires design approval before implementation). The iterate process has its own design gates (scope review, PAR) that replace interactive approval. Do not block on skills that assume a human is present to approve each step.
...
|---|---|---|
| Extract | `extract-requirements` | Chunk → parallel extract → aggregate → `requirements/` |
| Scope | `scope-core` | Walking skeleton + iterations → `roadmap.md` (with PAR scope review) |
| Implement | `run-iteration` | Scope review → decompose → `implement-tasks` → wrap up |
| Task execution | `implement-tasks` | Per-task: implementer → PAR spec review → PAR quality review |
| Audit | `audit-progress` | PAR paired auditors, two-tier (deep + sweep) |
