...
Resolve [skills/brainstorming/scripts/start-server.mjs](scripts/start-server.mjs) relative to this
...
#           "screen_dir":"/path/to/project/.moe/brainstorm/12345-1706000000/content",
#           "state_dir":"/path/to/project/.moe/brainstorm/12345-1706000000/state"}
```
...
**Finding connection info:** The server writes its startup JSON to `$STATE_DIR/server-info`. If you launched the server in the background and didn't capture stdout, read that file to get the URL and port. When using `--project-dir`, check `<project>/.moe/brainstorm/` for the session directory.

**Note:** Pass the project root as `--project-dir` so mockups persist in `.moe/brainstorm/` and survive server restarts. Without it, files go to `/tmp` and get cleaned up. Remind the user to add `.moe/` to `.gitignore` if it's not already there.
...
Resolve [skills/brainstorming/scripts/stop-server.mjs](scripts/stop-server.mjs) relative to this
