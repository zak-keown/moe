...
import { helperScript } from "./helper.mjs";
...
export const MAX_FRAME_PAYLOAD_BYTES = 10 * 1024 * 1024;
...
export function writeSecretFile(file, data, mode) {
	try {
		fs.unlinkSync(file);
	} catch (_e) {
		/* nothing there yet */
...
function brandMarkup() {
	...
	return (
		'<div class="brand"><span class="brand-copy">Moe v' +
		version +
		"</span></div>"
	);
}
...
function companionUrl() {
	return (
		"http://" + urlHostForHttp(URL_HOST) + ":" + PORT + "/?key=" + TOKEN
	);
}

export function browserLauncherForPlatform(
	url,
	{
...
function maybeOpenBrowser() {
	...
	if (process.env.BRAINSTORM_OPEN_CMD) {
		try {
			execFile(
				"sh",
				["-c", process.env.BRAINSTORM_OPEN_CMD + " " + JSON.stringify(url)],
				() => {},
			);
		} catch (_e) {
			/* best effort */
	...
	if (!launcher) return;
	try {
		execFile(launcher.bin, launcher.args, () => {});
	} catch (_e) {
		/* best effort */
	}
}
...
function startServer() {
	...
	function onListen() {
		...
		if (PORT_FILE && !triedFallback) {
			try {
				writeSecretFile(PORT_FILE, String(PORT), 0o600);
			} catch (_e) {
				/* best effort */
			}
			if (TOKEN_FILE) {
				try {
					writeSecretFile(TOKEN_FILE, TOKEN, 0o600);
				} catch (_e) {
					/* best effort */
				}
			}
