...
**Architecture and design tasks**: use `opus`.
The final whole-branch review is one of these — dispatch it with
`opus`, not an unexamined session default.

**Review tasks**: use the configured default for ordinary reviews and `opus`
when the diff's size, complexity, or risk needs deeper judgment. Scoped
re-reviews of small mechanical fix diffs may use `haiku`.
...
that implementer. Single-file mechanical fixes also use `haiku`.
...
with `opus` (see Model Selection), using
`request-review`'s
