...
function preferredPort() {
  ...
  if (PORT_FILE) {
    ...
    } catch (_e) { /* no prior port recorded */ }
  }
  return randomPort();
}
...
function initialToken() {
  ...
  if (TOKEN_FILE) {
    ...
    } catch (_e) { /* no prior token recorded */ }
  }
...
const helperScript = fs.readFileSync(path.join(__dirname, 'helper.cjs'), 'utf-8');
const helperInjection = '<script>\n' + helperScript + '\n</script>';

// ========== Helper Functions ==========

// __dirname is <plugin-root>/skills/brainstorming/scripts, so three levels up
// is the plugin root. moe-mint always emits .claude-plugin/plugin.json there;
// package.json is the fallback for a source-tree run (packages/core/).
...
// Text only, no remote asset and no outbound link. Upstream injected an <img>
// from primeradiant.com here, which phoned the version and the viewer's IP
// home on every session; it was opt-out, not opt-in. Removed on import.
function brandMarkup() {
  const version = escapeHtmlText(PLUGIN_VERSION);
  return '<div class="brand"><span class="brand-copy">Moe v' + version + '</span></div>';
}
...
function broadcast(msg) {
  ...
  for (const socket of clients) {
    try { socket.write(frame); } catch (_e) { clients.delete(socket); }
  }
}
...
function maybeOpenBrowser() {
  ...
  if (process.env.BRAINSTORM_OPEN_CMD) {
    try { cp.exec(process.env.BRAINSTORM_OPEN_CMD + ' ' + JSON.stringify(url), () => {}); } catch (_e) { /* best effort */ }
    return;
  }
  ...
  try { cp.execFile(launcher.bin, launcher.args, () => {}); } catch (_e) { /* best effort */ }
}
