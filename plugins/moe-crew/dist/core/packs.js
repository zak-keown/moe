 ...
 * `.json`.
 */
export function parsePackYaml(text) {
...
function parseScalar(raw) {
    ...
    const isQuoted = (trimmedRaw.startsWith('"') && trimmedRaw.endsWith('"') && trimmedRaw.length >= 2) ||
    ...
    if (stripped === "true" || stripped === "True" || stripped === "TRUE")
        return true;
    if (stripped === "false" || stripped === "False" || stripped === "FALSE")
        return false;
    ...
    if (stripped.length > 0 && !Number.isNaN(n) && /^-?[0-9]/.test(stripped))
        return n;
    return stripped;
}
/**
...
function validatePack(raw) {
    ...
    for (let i = 0; i < workers.length; i++) {
        ...
        if (typeof w.namePrefix !== "string" || w.namePrefix.trim().length === 0) {
            throw new Error(`Invalid pack file: workers[${i}].namePrefix is required and must be a non-empty string`);
        }
        if (typeof w.rolePrompt !== "string" || w.rolePrompt.trim().length === 0) {
            throw new Error(`Invalid pack file: workers[${i}].rolePrompt is required and must be a non-empty string`);
        }
        const pw = {
