 ...
 */
export async function cmdList(ctx, opts) {
    const rows = [];
    for (const meta of listWorkers(ctx.workerDir)) {
        if (opts.pattern && !meta.tmux_name.includes(opts.pattern))
            continue;
        const status = await computeStatus(ctx, meta);
        if (status === "gone" && !opts.all)
            continue;
        rows.push([
            status,
            meta.harness,
    ...
    for (const name of listOrphanNames(ctx.workerDir)) {
        if (opts.pattern && !name.includes(opts.pattern))
            continue;
        if (!(await ctx.tmux.hasSession(name)))
            continue; // dead leftover -> prune territory
        rows.push([
            "unregistered",
