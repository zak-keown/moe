...
import { ensureOwnedDir, listWorkers, readHarnessMarker, readMeta, writeMeta, writeShim, } from "../core/worker-store.js";
 ...
 */
function existingHarnessState(workerDir, tmuxName, sessionId) {
    const evidence = [];
    ...
    for (const meta of listWorkers(workerDir)) {
        if (meta.tmux_name !== tmuxName || seenSessionIds.has(meta.session_id))
            continue;
        seenSessionIds.add(meta.session_id);
        evidence.push({
