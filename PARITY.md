...
contains it. Product documentation should describe Moe itself; explicit
lineage belongs here and in the root `NOTICE`.
...
- Historical evidence may retain original names and URLs under `docs/history`,
  tests, fixtures, and `.planning`. Current product surfaces may not.
...
tests. The prior ledger incorrectly described the entire upstream repository as
landed; the tree contains only this bridgehead.
