...
import type { JournalThoughts, MemoryEdge, RelationType, SourceType } from "./types.js";
...
const JOURNAL_SCOPE_PROPERTY = {
  ...
  description:
    "project-specific notes, user-global notes, or both (default: both). A project entry belongs to this codebase; a user entry follows you between them.",
  default: "both",
} as const;
...
function toolDefinitions() {
  return [
    ...
    },
    {
      name: "search_journal",
      description:
        "Search through your private journal entries using natural language queries. Returns semantically similar entries ranked by relevance. Searches what you DELIBERATELY wrote down; for past conversation transcripts, use search_conversations.",
      inputSchema: {
        type: "object",
        properties: {
          query: {
            type: "string",
            description:
              "Natural language search query (e.g., 'times I felt frustrated with TypeScript', 'what I've learned about how my collaborator prefers to work', 'lessons about async patterns')",
          },
          limit: {
            type: "number",
          ...
          sections: {
            ...
            items: { type: "string" },
            description:
              "Filter by section, using the same names as process_thoughts: ['reflections', 'observations', 'project_notes', 'user_context', 'technical_insights', 'world_knowledge']. 'feelings' still matches pre-2.0.0 entries.",
          },
        },
        required: ["query"],
        additionalProperties: false,
    ...
    },
    {
      name: "link_memories",
      description:
        "Create a typed relationship between two memory records. Source and target are type:id strings (e.g. 'exchange:abc123', 'journal:def456'). Relations: caused_by, contradicts, supersedes, supports, implements.",
      inputSchema: {
        type: "object",
        properties: {
          source: {
            type: "string",
            description:
              "Source record as type:id (e.g. 'exchange:abc123', 'journal:def456', 'decision:ghi789')",
          },
          target: {
            type: "string",
            description:
              "Target record as type:id (e.g. 'exchange:abc123', 'journal:def456', 'finding:jkl012')",
          },
          relation: {
            type: "string",
    ...
    },
    {
      ...
      description:
        "Walk the relationship graph from a memory record. Returns the chain of connected records up to the specified depth. Use 'causes' to find what led to a record, 'effects' to find what it influenced.",
      inputSchema: {
        type: "object",
        properties: {
          id: {
          ...
          direction: {
            ...
            enum: ["causes", "effects"],
            description:
              "Direction to walk: 'causes' finds what led to this record, 'effects' finds what it influenced (default: 'causes')",
            default: "causes",
          },
        },
        required: ["id"],
    ...
    try {
      ...
      if (name === "search_conversations") {
        ...
        if (Array.isArray(params.query)) {
          ...
          const singleOptions: SearchOptions = {
            mode: params.mode,
            limit: params.limit,
            after: params.after,
            before: params.before,
      ...
      if (name === "process_thoughts") {
        ...
        const thoughts: JournalThoughts = params;
      ...
      if (name === "search_journal") {
        ...
        try {
          const search = new JournalSearchService(
            db,
            journalStore.roots().map((r) => r.path),
          );
          ...
          if (results.length === 0) return textResult("No relevant entries found.");

          const body = results
            .map(
              (result, i) =>
                `${i + 1}. [Score: ${result.score.toFixed(3)}] ${new Date(
                  result.entry.timestamp,
                ).toLocaleDateString()} (${result.entry.scope})\n` +
      ...
      if (name === "read_journal_entry") {
        ...
        try {
          ...
          if (content === null) throw new Error("Entry not found");
      ...
      if (name === "list_recent_entries") {
        ...
        try {
          ...
          const body = results
            .map(
              (result, i) =>
                `${i + 1}. ${new Date(result.entry.timestamp).toLocaleDateString()} (${
                  result.entry.scope
                })\n` +
                `   Sections: ${result.entry.sections.join(", ")}\n` +
                `   Path: ${result.entry.path}\n` +
      ...
      if (name === "read_recent_entries") {
        ...
        try {
          ...
          if (results.length === 0) return textResult("No recent entries found.");

          return textResult(
            results
              .map(
                (entry, i) =>
                  `--- Entry ${i + 1} (${new Date(entry.timestamp).toLocaleDateString()}, ${
                    entry.scope
                  }) ---\nPath: ${entry.path}\n\n${entry.content}`,
              )
              .join("\n\n"),
      ...
      if (name === "link_memories") {
        ...
        return textResult(
          `Linked ${params.source} --[${params.relation}]--> ${params.target} (edge ${edgeId}, confidence ${params.confidence})`,
        );
      }
...
export function parseJournalPathArg(argv: string[]): string | undefined {
...
if (process.argv[1] && import.meta.url === `file://${process.argv[1]}`) {
  runMemoryMcpServer().catch((error: unknown) => {
