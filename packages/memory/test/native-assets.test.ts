...
import { resolveInstalledPackageRoot, type InstalledPackageRoot } from "../src/installed-package-root.js";
...
describe("native asset manifest", () => {
	...
	it.each([
		...
		"linux-x64",
		"win32-x64",
	] as const)("accounts for %s once", (target) => {
	...
	it("verifies the local platform binary", () => {
		const target =
			`${process.platform}-${process.arch}` as keyof typeof manifest.targets;
	...
	it("rejects a tampered hash", () => {
		...
		expect(() => verifyNativeAsset(PACKAGE_ROOT, record)).toThrow(
			/SHA-256 mismatch/,
		);
	});
...
describe("resolveNativeAsset", () => {
	it("resolves the local platform binary", () => {
		...
		expect(asset.record.target).toBe(
	...
	it("throws for unsupported target", () => {
		expect(() =>
			resolveNativeAsset(PACKAGE_ROOT, "freebsd" as NodeJS.Platform, "x64"),
		).toThrow(/unsupported/);
	});
});
...
describe("fixture-based containment", () => {
	it("selects exactly one verified asset from a fixture package root", () => {
		const tmpRoot = mkdtempSync(
			resolve(tmpdir(), "native-asset-test-"),
