...
function readMintYaml(): string {
  return readFileSync(
    path.join(PACKAGE_ROOT, "mint/moe-memory.yaml"),
    "utf-8",
  );
}
...
describe("moe-memory artifact gate", () => {
  ...
  it("includes all required payload roots", () => {
    ...
    const roots = payloadLines
      ...
      .filter(Boolean)
      .sort();
    expect(roots).toEqual(["agents", "dist", "hooks", "prompts", "recovery", "runtime", "skills", "vendor/sqlite-vec"]);
  });
  ...
  it("ships runtime-contract.json at package root", () => {
    expect(existsSync(path.join(PACKAGE_ROOT, "runtime-contract.json"))).toBe(
      true,
    );
  });
