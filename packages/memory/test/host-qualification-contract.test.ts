...
const WORKFLOW_PATH = join(REPO_ROOT, ".github/workflows/memory-host-qualification.yml");
const ORCHESTRATOR_PATH = join(PACKAGE_ROOT, "test/manual/host-qualification.js");
...
describe("host qualification contract", () => {
  ...
  it("installs only the uploaded candidate tarball", () => {
    ...
    const nonCommentLines = workflow.split("\n").filter((l) => !l.trimStart().startsWith("#"));
  ...
  it("copilot compatibility script exists", () => {
    const copilotPath = join(REPO_ROOT, "packages/mint/test/manual/copilot-compatibility.js");
    expect(existsSync(copilotPath)).toBe(true);
  });

  it("claude e2e script supports tarball-based qualification", () => {
    const claudeE2e = readFileSync(join(PACKAGE_ROOT, "test/manual/claude-e2e.js"), "utf-8");
    expect(claudeE2e).toContain("MOE_MEMORY_E2E_PLUGIN_DIR");
  });

  it("codex e2e script supports tarball-based qualification", () => {
    const codexE2e = readFileSync(join(PACKAGE_ROOT, "test/manual/codex-e2e.js"), "utf-8");
