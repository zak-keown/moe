...
node scripts/chrome-ws.mjs start    # Auto-detects platform, launches Chrome
node scripts/chrome-ws.mjs tabs     # Verify running
```
...
CHROME_WS_BROWSER=/usr/bin/chromium node scripts/chrome-ws.mjs start

# Use custom port
CHROME_WS_PORT=9333 node scripts/chrome-ws.mjs start

# Use Brave browser
CHROME_WS_BROWSER="/usr/bin/brave-browser" node scripts/chrome-ws.mjs start
```
