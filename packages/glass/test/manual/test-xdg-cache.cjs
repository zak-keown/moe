...
// Verify structure: {cacheHome}/moe/browser/YYYY-MM-DD/session-{timestamp}
const parts = sessionDir.split(path.sep);
const hasMoe = parts.includes('moe');
...
console.log(`  Has 'moe' in path: ${hasMoe ? '✓' : '✗'}`);
...
if (hasMoe && hasBrowser && hasDatePattern && hasSessionPattern) {
  console.log('  ✓ Pass\n');
} else {
  console.log('  ✗ FAIL\n');
  process.exit(1);
