 ...
 *
 * Tests:
 * 1. Default profile is 'moe-glass'
...
const chromeLib = require('../../browsing-compat/chrome-ws-lib.js').createSession();
const fs = require('fs');
...
async function runTests() {
  ...
  try {
    ...
    if (defaultProfile === 'moe-glass') {
      console.log('  ✓ Pass: Default profile is "moe-glass"\n');
    } else {
      console.log(`  ✗ FAIL: Expected "moe-glass", got "${defaultProfile}"\n`);
      process.exit(1);
    }

    // Test 2: Check default profile directory path
    console.log('Test 2: Check default profile directory');
    const defaultProfileDir = chromeLib.getChromeProfileDir('moe-glass');
    console.log(`  Profile dir: ${defaultProfileDir}`);
    if (defaultProfileDir.includes('browser-profiles') && defaultProfileDir.includes('moe-glass')) {
    ...
    if (mode.profile === 'moe-glass' && fs.existsSync(mode.profileDir)) {
    ...
    const defaultDir = chromeLib.getChromeProfileDir('moe-glass');
    ...
    console.log('\n✅ All tests passed!');
    console.log('\nKey findings:');
    console.log('- Default profile is "moe-glass"');
    console.log('- Profiles are stored in ~/.cache/moe/browser-profiles/{name}/');
