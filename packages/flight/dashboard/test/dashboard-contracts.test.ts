...
import { cellId, cellKey, DashboardVerdictSchema, PhaseJsonSchema } from "../src/contracts.js";
...
test("PhaseJsonSchema requires pid (liveness signal)", () => {
  expect(PhaseJsonSchema.safeParse({ phase: "agent", updated_at: "x" }).success).toBe(false);
});
