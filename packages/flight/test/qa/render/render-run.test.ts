...
describe("renderRunFromTemplate", () => {
  ...
  test("throws if result.json is missing", async () => {
    ...
    mkdirSync(badRunDir);
    await expect(renderRunFromTemplate({ runDir: badRunDir, templatePath })).rejects.toThrow(
      /result\.json/,
    );
  });
  ...
  test("uses outputName override when provided", async () => {
    const { runDir, templatePath } = makeFixtureRun();
    const outPath = await renderRunFromTemplate({
      runDir,
      templatePath,
  ...
  test("throws if template is missing the placeholder script tag", async () => {
    ...
    writeFileSync(noPlaceholder, `<!doctype html><html><head></head><body></body></html>`);
    await expect(renderRunFromTemplate({ runDir, templatePath: noPlaceholder })).rejects.toThrow(
      /__MOE_FLIGHT_RUN__/,
    );
  });
});
