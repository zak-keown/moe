...
describe("PrettyRenderer", () => {
  ...
  test("spinner writes waiting line on llm_request and clears on next event (TTY/color on)", () => {
    const sink = collect();
    const r = new PrettyRenderer(sink, { color: true, columns: 100 });
    r.handle({ eventId: 1, parentEventId: 0, ts: "t", type: "run_start", runId: "r", cardId: "c", target: "t", provider: "a", model: "claude-sonnet-4-6", adapter: "web", budgetMs: 300_000, toolTimeoutMs: 1, contextTreeBytes: 0 } as any);
  ...
  test("assistant text longer than columns is soft-wrapped under the » glyph", () => {
    ...
    r.handle({ eventId: 1, parentEventId: 0, ts: "t", type: "run_start", runId: "r", cardId: "c", target: "t", provider: "a", model: "m", adapter: "cli", budgetMs: 300_000, toolTimeoutMs: 1, contextTreeBytes: 0 } as any);
