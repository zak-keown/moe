  ...
  test("returns systemPrompt, messages, modelId, adapterName for a 2-turn run", () => {
    const dir = makeRunDir([
      ...
      { type: "llm_request", turn: 1, messageCount: 1 },
      {
        ...
        rawAssistantMessage: {
          role: "assistant",
          content: [{ type: "tool_use", id: "t1", name: "click", input: { selector: "#login" } }],
        },
      },
      {
        type: "tool_call",
      ...
      },
      {
        ...
        observations: [],
        durationMs: 100,
        usage: { inputTokens: 100, outputTokens: 20, turns: 1 },
      },
    ]);
    cleanups.push(dir);
...
describe("rebuildMessages — recovery-turn fidelity", () => {
  test("a max_tokens recovery turn replays the stub, then the nudge — not the truncated raw (PRI-2160)", () => {
    const dir = makeRunDir([
      ...
      {
        type: "tool_definitions",
        tools: [{ name: "click", description: "Click", parameters: { type: "object" } }],
      },
      ...
      // pushed a stub, and nudged.
      {
        ...
        text: "I was reasoning at len",
        thinking: [],
        toolCalls: [],
        usage: { inputTokens: 10, outputTokens: 4096 },
        rawAssistantMessage: {
          role: "assistant",
          content: [{ type: "text", text: "I was reasoning at len" }],
        },
      },
      {
        type: "event",
        name: "stopped_max_tokens",
        turn: 1,
        hasText: true,
        toolCallCount: 0,
        recovery: true,
      },
      {
        type: "user_message",
        turn: 1,
        content: "<SYSTEM-REMINDER>cut off; be concise</SYSTEM-REMINDER>",
      },
      // Turn 2: recovered with a normal tool call.
      {
        ...
        rawAssistantMessage: {
          role: "assistant",
          content: [{ type: "tool_use", id: "t2", name: "click", input: {} }],
        },
      },
    ...
    const stubIdx = result.messages.findIndex((m) =>
      JSON.stringify(m).includes("truncated by the output token limit"),
    );
    const nudgeIdx = result.messages.findIndex((m) =>
      JSON.stringify(m).includes("cut off; be concise"),
    );
  ...
  test("an empty-response nudge turn replays the filled stub, then the nudge (PRI-1864)", () => {
    const dir = makeRunDir([
      ...
      { type: "event", name: "empty_response_nudge", turn: 1, stopReason: "end_turn" },
      {
        type: "user_message",
        turn: 1,
        content: "<SYSTEM-REMINDER>empty turn nudge</SYSTEM-REMINDER>",
      },
      {
        type: "llm_response",
        turn: 2,
    ...
    const nudgeIdx = result.messages.findIndex((m) =>
      JSON.stringify(m).includes("empty turn nudge"),
    );
    expect(stubIdx).toBeGreaterThanOrEqual(0);
    expect(nudgeIdx).toBe(stubIdx + 1);
  });
  ...
  test("a grace turn whose response happens to be empty keeps the user-before-assistant order", () => {
    ...
    const dir = makeRunDir([
      ...
      { type: "user_message", turn: 0, content: "go" },
      {
        type: "user_message",
        turn: 1,
        content: "<SYSTEM-REMINDER>time budget reminder</SYSTEM-REMINDER>",
      },
      {
        type: "llm_response",
        turn: 1,
    ...
    const reminderIdx = result.messages.findIndex((m) =>
      JSON.stringify(m).includes("time budget reminder"),
    );
...
describe("rebuildMessages — image rehydration", () => {
  test("reads screenshot bytes from disk and slots them into the tool_result block", () => {
    const dir = makeRunDir([
      ...
      {
        type: "tool_definitions",
        tools: [{ name: "screenshot", description: "shoot", parameters: { type: "object" } }],
      },
      { type: "user_message", turn: 0, content: "go" },
      {
        type: "llm_response",
  ...
  test("weaves the reflection reminder into the same user turn as tool_result (no separate user message)", () => {
    const dir = makeRunDir([
      ...
      { type: "user_message", turn: 0, content: "go" },
      {
        ...
        rawAssistantMessage: {
          role: "assistant",
          content: [{ type: "tool_use", id: "t1", name: "click", input: {} }],
        },
      },
      ...
      { type: "event", name: "reflection_checkpoint", turn: 1, ordinal: 1, traceLength: 1 },
      {
        type: "user_message",
        turn: 1,
        content: "<SYSTEM-REMINDER> reflect now </SYSTEM-REMINDER>",
      },
    ]);
    cleanups.push(dir);
...
describe("rebuildMessages — terminal tool_use stub", () => {
  test("synthesizes a tool_result user turn for report_result on the final assistant message", () => {
    const dir = makeRunDir([
      ...
      {
        type: "tool_definitions",
        tools: [{ name: "report_result", description: "report", parameters: { type: "object" } }],
      },
      { type: "user_message", turn: 0, content: "go" },
      {
        type: "llm_response",
...
describe("rebuildMessages — TUI capture rehydration", () => {
  test("reads the .ansi file when capturePath is set", () => {
    const dir = makeRunDir([
      ...
      {
        type: "tool_definitions",
        tools: [{ name: "read_screen", description: "read", parameters: { type: "object" } }],
      },
      { type: "user_message", turn: 0, content: "go" },
      {
        type: "llm_response",
        ...
        rawAssistantMessage: {
          role: "assistant",
          content: [{ type: "tool_use", id: "t1", name: "read_screen", input: {} }],
        },
      },
...
describe("rebuildMessages — old-run fallback", () => {
  ...
  test("errors with a clear message when the recorded adapter is no longer registered", () => {
    ...
    cleanups.push(dir);
    expect(() => rebuildMessages(dir, makeFakeAnthropicClient())).toThrow(
      /not registered|no longer/i,
    );
  });
});
...
describe("rebuildMessages — --turn cutoff", () => {
  const events3turn = [
    ...
    },
    {
      ...
      observations: [],
      durationMs: 100,
      usage: { inputTokens: 10, outputTokens: 5, turns: 3 },
    },
  ];
  ...
  test("--turn out of range errors clearly", () => {
    const dir = makeRunDir(events3turn);
    cleanups.push(dir);
    expect(() => rebuildMessages(dir, makeFakeAnthropicClient(), 99)).toThrow(
      /out of range|ended at turn 3/,
    );
  });
});
