...
describe("Web e2e — form-post + return_screenshot (PRI-1517 T2b)", () => {
  test("click(button, return_screenshot:true) returns a truthful result well under the 30s regression", async () => {
    let WebAdapter: any;
    try {
    ...
    try {
      ...
      const events = readFileSync(join(logDir, "run.jsonl"), "utf-8")
        ...
        .split("\n")
        .map((l) => JSON.parse(l));
      const clickResult = events.find((e: any) => e.type === "tool_result" && e.name === "click");
