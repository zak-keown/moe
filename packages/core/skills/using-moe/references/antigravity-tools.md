...
On Antigravity, rung 1 of the shared ladder in {resource:skills/_shared/native-rendering.md}, resolved relative to this loaded document, is not exposed — skills that render should start at rung 2 (the brainstorm browser companion) or fall to rung 4 (markdown file); rung 3 is not a reliable middle step here. Antigravity's `write_to_file` with `IsArtifact: true` is a task-artifact concept and is not a substitute for rung 1.
