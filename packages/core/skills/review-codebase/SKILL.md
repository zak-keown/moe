...
| | files | lenses | model |
|---|---|---|---|
| `shallow` | entrypoints, plus files changed most often in git history | correctness, security | {model-default} |
| `medium` | all tracked source after exclusions | plus tests, error handling, API contracts | {model-default} |
| `deep` | plus tests, config, scripts, infrastructure | plus performance, coupling, dependency risk | **{model-deep}** |
...
Resolve {resource:skills/review-codebase/scripts/review-scope.mjs} relative to this loaded document, then invoke it as:
...
As reports land, resolve {resource:skills/review-codebase/scripts/review-check.mjs} relative to this loaded document and validate each one before the merge ever sees it:
...
Finally, resolve {resource:skills/review-codebase/scripts/review-merge.mjs} relative to this loaded document:
...
`CR-###` IDs. Resolve {resource:skills/review-codebase/scripts/review-verify-scope.mjs} relative to this loaded document, then split them out:
...
Resolve {resource:skills/review-codebase/scripts/review-verify-record.mjs}
