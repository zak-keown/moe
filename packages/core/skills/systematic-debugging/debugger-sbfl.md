...
A deterministic ranked "where to look first" list, derived from existing test pass/fail coverage before any reasoning happens. Slots between Phase 1 (Root Cause Investigation) and Phase 3 (Hypothesis and Testing) — after evidence, before hypotheses.
...
Phase 1 (Root Cause Investigation) searches the codebase and reads files to seed
...
so cap it (60s for npm-scale suites; scale with suite size) and **degrade to
