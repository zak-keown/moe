...
```bash
node "$SKILL/review-verify-scope.mjs" \
...
Resolve {resource:skills/reviewing-a-codebase/scripts/review-verify-record.mjs}
relative to this loaded document and record each reply as it arrives, from the
saved reply or the bare object:

```bash
node "$SKILL/review-verify-record.mjs" \
