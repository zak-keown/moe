...
- Each plan owns a workspace: at skill start, resolve
  {resource:skills/sdd/scripts/sdd-workspace.mjs} relative
...
waves deterministically. Resolve {resource:skills/sdd/scripts/task-set.mjs} relative to this loaded document, then invoke it as:
...
**Mechanical implementation tasks** (isolated functions, clear specs, 1-2 files): use {model-fast}. Most implementation tasks are mechanical when the plan is well-specified.

**Integration and judgment tasks** (multi-file coordination, pattern matching, debugging): use {model-default}.

**Architecture and design tasks**: use {model-deep}.
The final whole-branch review is one of these — dispatch it with
{model-deep}, not an unexamined session default.

**Review tasks**: use {model-default} for ordinary reviews and {model-deep}
when the diff's size, complexity, or risk needs deeper judgment. Scoped
re-reviews of small mechanical fix diffs may use {model-fast}.
...
turns on multi-step work — costing more overall. Use {model-default} as the
floor for reviewers and for implementers working from prose descriptions.
When the task's plan text contains the complete code to write, the
implementation is transcription plus testing: use {model-fast} for
that implementer. Single-file mechanical fixes also use {model-fast}.

**Task complexity signals (implementation tasks):**
- Touches 1-2 files with a complete spec → {model-fast}
- Touches multiple files with integration concerns → {model-default}
- Requires design judgment or broad codebase understanding → {model-deep}
...
- **Task brief:** before dispatching an implementer, resolve
  {resource:skills/sdd/scripts/task-brief.mjs} relative to
...
with {model-deep} (see Model Selection), using
`request-review`'s
