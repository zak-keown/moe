...
> **Authoritative source:** `packages/core/hooks/run-hook.cmd` is the canonical implementation. When this document and the code diverge, trust the code.
...
└── claude-judge-continuation   # Actual hook logic — extensionless bash script
```

In this package `hooks/moe-mint/` also exists at install time: moe-mint emits
its own `session-start` + `run-hook.cmd` pair there for the bootstrap skill, at
a depth of two below the plugin root rather than one. Do not confuse the two —
the generated pair is not hand-editable.
    ...
    "Stop": [
      {
        "hooks": [
          {
            "type": "command",
            "command": "\"${CLAUDE_PLUGIN_ROOT}/hooks/run-hook.cmd\" claude-judge-continuation",
            "shell": "bash",
            "async": false
          }
        ]
...
Check that the script filename is **extensionless** in `hooks.json`. A command like `run-hook.cmd claude-judge-continuation.sh` can trigger Claude Code's `.sh` auto-detection and bypass the intended CMD dispatcher path, or just try to run a non-existent `.sh` script.

### Hook doesn't fire at all

Verify the `matcher` in `hooks.json` matches the event type your harness emits. Claude Code uses `startup|clear|compact`; Cursor uses `sessionStart`. moe-mint's cursor adapter emits the Cursor variant; you do not write it.
