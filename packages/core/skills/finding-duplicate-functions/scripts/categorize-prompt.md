...
Use this prompt with a **{model-fast}** subagent for cost-effective categorization.
...
1. Run extraction: resolve {resource:skills/finding-duplicate-functions/scripts/extract-functions.mjs} relative to this loaded document, then run `node "<resolved-extract-functions.mjs>" src/ -o catalog.json`
2. Dispatch {model-fast} subagent with the prompt above, replacing:
