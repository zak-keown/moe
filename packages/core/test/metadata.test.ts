...
describe("the skill registry", () => {
  ...
  // `plugins/` is generated by `pnpm mint` and is deliberately NOT regenerated
  // as part of the skill-renaming task (that is Task 3's job), so it still
  // ships the pre-rename directory names below. This map exists ONLY to
  // translate the (renamed) `skillNames` back to their pre-rename form for
  // this generated-root check; once mint regenerates `plugins/` with the new
  // names, this map — and its use just below — should be deleted and the
  // check pointed straight at `skillNames`.
  const STALE_GENERATED_SKILL_NAMES: Record<string, string> = {
    tdd: "test-driven-development",
    sdd: "subagent-driven-development",
    "verify-completion": "verification-before-completion",
    "write-plan": "writing-plans",
    "execute-plan": "executing-plans",
    "use-worktrees": "using-git-worktrees",
    "finish-branch": "finishing-a-development-branch",
    "request-review": "requesting-code-review",
    "receive-review": "receiving-code-review",
    "write-clearly": "writing-clearly-and-concisely",
    "dispatch-agents": "dispatching-parallel-agents",
    "write-skill": "writing-skills",
    "cc-config": "working-with-claude-code",
    "cc-plugins": "developing-claude-code-plugins",
    "find-duplicates": "finding-duplicate-functions",
    "use-tmux": "using-tmux-for-interactive-commands",
    iterate: "iterative-development",
    "extract-requirements": "extracting-requirements",
    "scope-core": "scoping-the-simplest-core",
    "run-iteration": "running-an-iteration",
    "implement-tasks": "implementing-tasks",
    "audit-progress": "auditing-progress",
    "improve-architecture": "improve-codebase-architecture",
    "resolve-conflicts": "resolving-merge-conflicts",
    "retrieve-context": "retrieving-context",
    "sequence-plans": "sequencing-plans",
    "review-codebase": "reviewing-a-codebase",
    "fix-review": "fixing-a-code-review",
    "moe-dev": "developing-for-moe",
    "smooth-experience": "smoothing-the-experience",
  };
  ...
  it("emits every skill into the moe plugin, plus _shared", () => {
    ...
    const staleSkillNames = [...skillNames].map(
      (name) => STALE_GENERATED_SKILL_NAMES[name] ?? name,
    );
    expect(emitted).toEqual(["_shared", ...staleSkillNames].sort());
  });
});
