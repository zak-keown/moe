...
function assertStartedOnExpectedPort(out) {
  ...
  assert.strictEqual(
    msg.port,
    TEST_PORT,
    `auth.test.cjs expected fixed port ${TEST_PORT}, got ${msg.port}; fixed-port tests must not run through fallback`
  );
  return msg;
}
...
async function runTests() {
  ...
  let _stdoutAccum = '';
  server.stdout.on('data', (d) => { _stdoutAccum += d.toString(); });
