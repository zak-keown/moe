...
// Fixed session key so the test client can authenticate (see auth.test.cjs for
...
function assertStartedOnExpectedPort(out) {
  ...
  assert.strictEqual(
    msg.port,
    TEST_PORT,
    `server.test.cjs expected fixed port ${TEST_PORT}, got ${msg.port}; fixed-port tests must not run through fallback`
  );
  return msg;
}
...
function ensureSymlinkWorks(target, link) {
  try {
    ...
    try { fs.unlinkSync(link); } catch (_ignore) {}
...
async function runTests() {
  ...
  try {
    ...
    await test('injects helper.cjs into waiting page', async () => {
      const res = await fetch(`http://localhost:${TEST_PORT}/`);
      assert(res.body.includes('WebSocket'), 'Should have helper.cjs injected');
    ...
    await test('serves full HTML documents as-is (not wrapped)', async () => {
      ...
      assert(res.body.includes('WebSocket'), 'Should still inject helper.cjs');
    ...
    await test('helper.cjs defines required APIs', () => {
      const helperContent = fs.readFileSync(
        path.join(__dirname, '../../skills/brainstorming/scripts/helper.cjs'), 'utf-8'
      );
