 ...
 *
 * Reads the report-base.html template from skills/_shared/ (or a
 * caller-supplied override), fills its slot markers from a JSON input
 * file, and writes self-contained HTML.
 *
...
function parseArgs(argv) {
  ...
  // Default template: skills/_shared/report-base.html relative to this script.
  if (!args.template) {
    args.template = path.join(__dirname, "..", "skills", "_shared", "report-base.html");
  }
  return args;
}
