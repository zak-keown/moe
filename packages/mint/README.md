...
| container image | `ghcr.io/prime-radiant-inc/everyharness-container` | `registry.gitlab.tcdevops.com/bubstack/moe/moe-container` |
...
**The container image reference is an assumption, not a fact.** Nothing has been
pushed to `registry.gitlab.tcdevops.com`. ARCHITECTURE.md §8 flags the project
path `bubstack/moe` as unconfirmed, and the registry hostname for a self-hosted
GitLab is a per-instance setting. `moe-mint test --image <ref>` overrides it.
...
`gitlab.tcdevops.com/bubstack/moe` would claim the fixture is Moe, and it would
