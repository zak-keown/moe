...
import { adjustedModel, type TargetId } from './vocabulary.js'
...
function installDocFile(model: PluginModel, adapter: HarnessAdapter): GeneratedFile | undefined {
  ...
  // installDoc must see the same effective component paths emit() does —
  // adjustedModel() relocates config.components.skills (and each skill's
  // dir) to the adapter's own skillLayout.outputDir before emit() runs, so
  // reading the un-adjusted base model here would describe a layout the
  // adapter never actually produces (CR-061).
  const body = adapter.installDoc(adjustedModel(model, adapter.skillLayout))
  const content =
    [
      GENERATED_MARKER,
      '',
