...
import { loadRuntimeContract, type RuntimeContractV1 } from '../src/runtime-contract.js'
...
describe('runtime-contract schema', () => {
  it('validates the memory runtime contract against the JSON Schema', () => {
  ...
  it('rejects unknown keys via additionalProperties: false', () => {
    const schema = JSON.parse(readFileSync(SCHEMA_PATH, 'utf-8'))
    const ajv = new Ajv({ strict: true })
    const validate = ajv.compile(schema)
