...
|---|---|---|---|
| Claude Code | automated | automated | automated |
| GitHub Copilot CLI | automated | manual | manual |
| Cursor, Codex, Kimi, OpenCode, Pi, Agent Plugins 1.0 | manual | manual | manual |

Manual plans contain the adapter's generated-equivalent instructions and exact
plugin list. Passing `--apply` to a manual route exits 2 before doctor probes,
host commands, or state changes.
...
otherwise silently skips. Pnpm is a soft contributor-workflow probe; generated
plugins do not need it at runtime.
...
Claude automation registers the canonical marketplace at
`https://github.com/zak-keown/moe.git` and manages each supported
`<plugin>@moe`. Copilot's verified install route uses the adapter-emitted
`https://github.com/zak-keown/moe` repository form. Claude Code alone accepts
`--scope user|project|local`; scopes on other harnesses are rejected with exit
2 rather than forwarded speculatively.

Removing the Claude marketplace from its last scope also removes its plugins.
If you want to remove only one plugin, use the selected host's plugin manager
directly.
