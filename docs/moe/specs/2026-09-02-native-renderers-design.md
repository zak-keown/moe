...
the achieved tree. A plugin with no skill files reports `unsupported` for every
adapter because there is no delivered tree to claim. OpenCode and Pi retain one
byte-identical root `package.json`,
